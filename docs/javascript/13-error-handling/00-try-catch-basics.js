 ...
 *
 * STORY: Imagine you're a tightrope walker (your code) performing high above
 * the ground. The try block is the tightrope -- where you do the risky walk.
 * The catch block is the safety net below -- if you slip (error), you land
 * safely instead of crashing to the ground. The finally block is the cleanup
 * crew that always comes out after the act, whether you fell or not -- they
 * pack up the equipment no matter what happened.
 *
 * ╔════════════════════════════════════════════════════════════════════════════╗
 * ║ THE GOLDEN RULE                                                            ║
 * ╠════════════════════════════════════════════════════════════════════════════╣
 ...
 * A: try-catch-finally is JS's error handling mechanism. The `try` block contains
 *    code that might throw. If an error occurs, execution jumps to `catch`, where
 *    we handle it gracefully. The `finally` block always runs — whether there was
 *    an error or not — perfect for cleanup like closing connections or files.
 *
 * Q: "What are the limitations of try-catch?"
 * A: (1) Only catches synchronous errors — async errors need `.catch()` or
 *    `try` with `await`. (2) Can't catch syntax errors (they happen at parse time).
 *    (3) Errors propagate up the call stack if not caught. (4) Since ES2019, the
 *    catch parameter is optional: `catch { }` instead of `catch(e) { }`.
 *
 * Q: "What properties does an Error object have?"
 * A: `name` (error type like TypeError, RangeError), `message` (human-readable
 *    description), and `stack` (call stack trace for debugging). Custom errors
 *    can extend the Error class to add more properties.
 *
 *
 * RUN: node docs/javascript/13-error-handling/00-try-catch-basics.js
 */
