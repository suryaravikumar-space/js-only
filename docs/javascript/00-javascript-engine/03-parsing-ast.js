 ...
 *
 * ╔════════════════════════════════════════════════════════════════════════════╗
 * ║ STORY: THE BLUEPRINT ARCHITECT                                            ║
 * ╠════════════════════════════════════════════════════════════════════════════╣
 * ║                                                                            ║
 * ║ Imagine you're building a house. The material supplier delivers a flat     ║
 * ║ list of items: [bricks] [cement] [beams] [glass] [wires] [pipes].         ║
 * ║ That's what TOKENS are — a flat, linear list with no structure.            ║
 * ║                                                                            ║
 * ║ Now the ARCHITECT (the Parser) takes that list and creates a BLUEPRINT    ║
 * ║ (the AST) — showing HOW everything connects:                              ║
 * ║   - Which walls support which floors                                      ║
 * ║   - Which wires go inside which walls                                     ║
 * ║   - Which pipes connect to which rooms                                    ║
 * ║                                                                            ║
 * ║ Without the blueprint, you just have a pile of materials. With it,        ║
 * ║ you have a HIERARCHY that shows structure and relationships.              ║
 * ║                                                                            ║
 * ║ Tokens: [var] [x] [=] [10] [+] [5] [;]                                    ║
 * ║                        ↓                                                   ║
 * ║                     PARSER                                                 ║
...
function tokenize(code) {
  const tokens = [];
  let current = 0;
  const keywords = ['var', 'let', 'const', 'function', 'return', 'if', 'else', 'true', 'false', 'null'];
...
function createParser(tokens) {
  ...
  function parseVariableDeclaration() {
    const kind = consume('KEYWORD').value; // var, let, or const
    const declarations = [];
  ...
  function parseIfStatement() {
    ...
    const consequent = peek().value === '{' ? parseBlockStatement() : parseStatement();
    let alternate = null;
  ...
  function parseCallMember() {
    ...
    while (true) {
      if (peek().type === 'PUNCTUATOR' && peek().value === '(') {
        ...
        object = { type: 'MemberExpression', object, property: { type: 'Identifier', name: property.value }, computed: false };
      } else if (peek().type === 'PUNCTUATOR' && peek().value === '[') {
        consume('PUNCTUATOR', '[');
...
console.log('═'.repeat(70));
const code5 = 'if (x > 10) { return true; } else { return false; }';
 ...
 * ║                                                                            ║
 * ║ A: An Abstract Syntax Tree is a tree representation of source code        ║
 * ║    structure. Unlike a flat list of tokens, the AST captures hierarchy    ║
 * ║    — operator precedence, nesting, scope. It's important because tools    ║
 * ║    like Babel (transpiling), ESLint (linting), Prettier (formatting),     ║
 * ║    and V8 itself (code generation) all work by manipulating the AST.      ║
 * ║                                                                            ║
 * ║ Q2: "How does the parser handle operator precedence like 2 + 3 * 4?"     ║
 * ║                                                                            ║
 * ║ A: The parser uses precedence climbing (recursive descent). It has        ║
 * ║    separate functions for each precedence level — parseAdditive for +/-,  ║
 * ║    parseMultiplicative for *//%. Since parseMultiplicative is called      ║
 * ║    inside parseAdditive, * binds tighter. So `2 + 3 * 4` becomes         ║
 * ║    `2 + (3 * 4)` in the AST, giving 14 not 20.                           ║
 * ║                                                                            ║
 * ║ Q3: "What's the difference between a syntax error and a semantic error?"  ║
 * ║                                                                            ║
 * ║ A: Syntax errors are caught by the parser — invalid structure like        ║
 * ║    `var = 10;` or missing parentheses. Semantic errors have valid syntax  ║
 * ║    but wrong meaning — like `const x = 1; x = 2;` (assignment to const)  ║
 * ║    or using an undeclared variable. These are caught later at compile     ║
 * ║    time or runtime.                                                       ║
 * ║                                                                            ║
 * ╚════════════════════════════════════════════════════════════════════════════╝
 *
