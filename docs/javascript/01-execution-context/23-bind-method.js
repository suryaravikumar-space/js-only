...
var bob = { name: 'Bob' };         // ES6: use `const`
var charlie = { name: 'Charlie' }; // ES6: use `const`

var boundToBob = person.greet.bind(bob); // ES6: use `const`

console.log('A:', boundToBob('Hello'));
console.log('B:', boundToBob.call(charlie, 'Hi'));
console.log('C:', boundToBob.apply(person, ['Hey']));

var doubleBound = boundToBob.bind(charlie); // ES6: use `const`
console.log('D:', doubleBound('Yo'));

var boundWithArg = person.greet.bind(bob, 'Fixed'); // ES6: use `const`
 ...
 * INTERVIEW QUESTIONS:
 *
 * Q: "Can you change `this` on a bound function using call() or apply()?"
 * A: No. bind() is PERMANENT. Once you bind a function to a specific `this`
 *    value, that binding cannot be changed by call(), apply(), or even another
 *    bind(). bind() also supports partial application -- you can pre-fill
 *    arguments. The only thing that can override bind() is the `new` keyword,
 *    which creates a new object and uses that as `this` instead.
 *
 *
