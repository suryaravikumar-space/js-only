...
var another = { value: 100 }; // ES6: use `const`
...
var numbers = [5, 10, 15]; // ES6: use `const`
 ...
 * A: apply() is identical to call() -- both invoke a function with a specific
 *    `this` value. The only difference is how arguments are passed: call() takes
 *    arguments individually (comma-separated), apply() takes arguments as an array.
 *    A common gotcha is using apply(null, ...) when the function needs `this`.
 *    In non-strict mode, null becomes the global object, so this.property will be
 *    undefined. null is safe when the function doesn't use `this` at all, like
 *    Math.max or Math.min. In modern ES6+, the spread operator often replaces
 *    apply(): Math.max(...numbers) instead of Math.max.apply(null, numbers).
 *
 *
 * ES6 NOTE: The spread operator (...) largely replaces apply() for passing arrays
 *           as arguments: Math.max(...numbers). Use rest parameters (...args)
 *           instead of the `arguments` object. Use `const`/`let` instead of `var`.
 *
 * RUN: node docs/javascript/01-execution-context/22-apply-method.js
 */
