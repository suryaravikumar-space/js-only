 ...
 * A: `var` allows redeclaration — `var a = 1; var a = 2;` works silently.
 *    `let` and `const` do NOT — you get a SyntaxError at parse time,
 *    before any code runs. This is an ES6 improvement to catch bugs early.
 *
 * Q: "What's the difference between SyntaxError, TypeError, and ReferenceError?"
 * A: SyntaxError: Invalid code structure — caught at parse time (before execution).
 *    Example: `let x; let x;` or `if (`.
 *    TypeError: Value used in wrong way — caught at runtime.
 *    Example: `undefined()` or `null.property`.
 *    ReferenceError: Variable not found or in TDZ — caught at runtime.
 *    Example: `console.log(undeclaredVar)` or accessing `let` before declaration.
 *
 * RUN: node docs/javascript/01-execution-context/04-redeclaration.js
 */
