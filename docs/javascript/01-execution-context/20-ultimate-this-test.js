...
var obj = { // ES6: use `const`
  name: 'Object',

  regular: function() {
    console.log('A:', this.name);

    var inner = function() { // ES6: use `const`
      console.log('B:', this.name);
    };
    inner();

    var arrow = () => { // ES6: use `const`
...
var name = 'Global'; // ES6: use `const` (var used here to demonstrate global `this` binding)
obj.regular();
 ...
 * INTERVIEW QUESTIONS:
 *
 * Q: "Explain the output when a regular function, an inner regular function, and an arrow function all log `this.name`."
 * A: A prints 'Object' because regular() is called as a method (obj.regular()),
 *    so this = obj. B prints 'undefined' because inner() is a regular function
 *    called standalone -- regular functions get their this based on HOW they're
 *    called, and standalone call means this = global (global.name is undefined
 *    in Node.js). C prints 'Object' because arrow() is an arrow function that
 *    inherits this from the scope WHERE it's DEFINED. arrow() is defined inside
 *    regular() where this = obj. The key difference: regular functions decide
 *    this at CALL time, arrow functions decide this at DEFINITION time.
 *
 *
 * ┌────────────────────────────────────────────────────────────────┐
 * │ CONGRATULATIONS!                                              │
 ...
 *
 *
 * ES6 NOTE: Use `const`/`let` instead of `var`, arrow functions for callbacks,
 *           and ES6 classes instead of constructor functions. Template literals
 *           can replace string concatenation.
 *
 * RUN: node docs/javascript/01-execution-context/20-ultimate-this-test.js
 */
