 ...
 * ╠════════════════════════════════════════════════════════════════════════════╣
 * ║                                                                            ║
 * ║ JavaScript uses JIT (Just-In-Time) Compilation - a hybrid approach:        ║
 * ║                                                                            ║
 * ║   INTERPRETER (Ignition): Fast startup, slower execution                   ║
 * ║   COMPILER (TurboFan):    Slow startup, faster execution                   ║
 * ║                                                                            ║
 * ║ The engine monitors "HOT" code (frequently executed) and compiles          ║
 * ║ only that code to optimized machine code. This is the best of both worlds. ║
 * ║                                                                            ║
 * ║ V8's Pipeline:                                                             ║
...
var startStable = Date.now();
var sumStable = 0;
for (var i = 0; i < 1000000; i++) {
  sumStable = addNumbersStable(i, 1);
}
var timeStable = Date.now() - startStable;
...
var startUnstable = Date.now();
var sumUnstable = 0;
for (var i = 0; i < 1000000; i++) {
  sumUnstable = addAnything(i, 1);
}
var timeUnstable = Date.now() - startUnstable;
