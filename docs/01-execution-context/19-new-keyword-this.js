 ...
 * ├────────────────────────────────────────────────────────────────┤
 * │                                                                │
 * │ "Person('John') without `new` is a standalone function call.  │
 * │  In non-strict mode, `this` is the global object. So          │
 * │  this.name = 'John' actually sets global.name = 'John',       │
 * │  polluting the global namespace. The function returns         │
 * │  nothing, so p1 is undefined.                                 │
 * │                                                                │
 * │  new Person('Jane') with `new` is different. The `new`        │
 * │  keyword creates a fresh empty object, sets `this` to that    │
 * │  object, runs the function, and automatically returns the     │
 * │  object. So p2 is { name: 'Jane' }.                           │
 * │                                                                │
 * │  console.log(name) prints 'John' because Person('John')       │
 * │  accidentally created a global variable.                      │
 * │                                                                │
 * │  This is why forgetting `new` is dangerous. Strict mode       │
 * │  catches this by making `this` undefined, causing an error.   │
 * │  ES6 classes also prevent this by requiring `new`."           │
 * │                                                                │
 * └────────────────────────────────────────────────────────────────┘
 *
 *
 * RUN: node docs/01-execution-context/19-new-keyword-this.js
 */
