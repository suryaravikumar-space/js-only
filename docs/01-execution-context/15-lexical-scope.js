 ...
 * ├────────────────────────────────────────────────────────────────┤
 * │                                                                │
 * │ "JavaScript uses lexical scoping, which means a function's    │
 * │  scope is determined by where it's DEFINED in the source      │
 * │  code, not where it's called. In this example, inner() is     │
 * │  defined at the global level, so its parent scope is global   │
 * │  where x = 10. Even though inner() is called from outer()     │
 * │  where x = 20, it still looks up to global and finds 10.      │
 * │  This is different from dynamic scoping where the call        │
 * │  location would matter."                                      │
 * │                                                                │
 * └────────────────────────────────────────────────────────────────┘
 *
 *
 * RUN: node docs/01-execution-context/15-lexical-scope.js
 */
