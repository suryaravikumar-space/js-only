 ...
 * ├────────────────────────────────────────────────────────────────┤
 * │                                                                │
 * │ "regularMethod prints 'John' because regular functions        │
 * │  determine `this` at call time — since it's called as         │
 * │  person.regularMethod(), `this` is person.                    │
 * │                                                                │
 * │  arrowMethod prints undefined (Node.js) or 'Global' (browser) │
 * │  because arrow functions inherit `this` from the lexical      │
 * │  scope where they're defined. Object literals don't create    │
 * │  scope, so the parent scope is global, not person.            │
 * │                                                                │
 * │  This is a key reason why arrow functions shouldn't be used   │
 * │  as object methods — they lose the object reference. But      │
 * │  they're perfect for callbacks inside methods where you       │
 * │  want to preserve the outer `this`."                          │
 * │                                                                │
 * └────────────────────────────────────────────────────────────────┘
 *
 *
 * RUN: node docs/01-execution-context/17-arrow-function-this.js
 */
