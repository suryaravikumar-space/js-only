 ...
 * ├────────────────────────────────────────────────────────────────┤
 * │                                                                │
 * │ "greetWithRegular prints 'undefined' because the callback     │
 * │  is a regular function. When setTimeout calls it later, it    │
 * │  calls it as a standalone function: callback(). Regular       │
 * │  functions determine `this` at call time, and a standalone    │
 * │  call means `this` is the global object (or undefined in      │
 * │  strict mode).                                                 │
 * │                                                                │
 * │  greetWithArrow prints 'John' because the callback is an      │
 * │  arrow function. Arrow functions don't have their own `this`  │
 * │  — they inherit it from the scope where they're defined.      │
 * │  The arrow is defined inside greetWithArrow where `this` is   │
 * │  person, so the arrow's `this` is also person.                │
 * │                                                                │
 * │  This is exactly why arrow functions were created — to        │
 * │  preserve `this` in callbacks. Before ES6, we had to use      │
 * │  `var self = this` or `.bind(this)` as workarounds."          │
 * │                                                                │
 * └────────────────────────────────────────────────────────────────┘
 *
 *
 * RUN: node docs/01-execution-context/18-arrow-inside-method.js
 */
